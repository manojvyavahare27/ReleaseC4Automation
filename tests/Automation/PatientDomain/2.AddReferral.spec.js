...
test.describe("Login Tests", () => {
  jsonData.addPatient.forEach(async (data, index) => {
    ...
    var sqlQuery = "select pat_id from patients where pat_hospital_ref= '" +  jsonData.addPatient[index].pat_hospital_ref + "' order by pat_id desc limit 1";
  ...
  console.log("Patient id is:" + patId);  
    var sqlQuery = "select * from referrals join referral_treatment_target_times on rtt_ref_id = ref_id where ref_pat_id ='" + patId +
    ...
    //// const query ="select add_email, add_mobile, add_phone,pat_name_other_lang, pat_need_interpreter_at_appointments from patients join addresses on pat_add_id = add_id where pat_hospital_ref= '" + data.pat_hospital_ref +"'";
    const patSqlFile ="SQLResults/PatientDomain/patientReferralDetails.json";
      //"D:\\Riomed\\Cellma4Automation\\SQLResults\\PatientDomain\\patientReferralDetails.json";
    results = await executeQuery(query, patSqlFile);
    console.log("\n Patient Details stored into the database: \n", results);
    const jsonFilePath1 = "SQLResults/PatientDomain/patientReferralDetails.json";
    //"D:\\Riomed\\Cellma4Automation\\SQLResults\\PatientDomain\\patientReferralDetails.json";
      // "D:\\Riomed\\Cellma4Automation\\SQLResults\\PatientDomain\\patientReferralDetails.json";
